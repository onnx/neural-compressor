...
class TestGPTQQuantWithInternalAPI(TestGPTQQuant):
    ...
    def test_gptq_with_QDQ_format(self):
        quant_config = config.GPTQConfig(
            weight_bits=4,
            weight_dtype="int",
            weight_sym=False,
        ...
        qmodel = algos.gptq_quantize_entry(op21_model, quant_config, calibration_data_reader=self.matmul_data_reader)
...
class TestGPTQQuantWithORTLikeAPI(TestGPTQQuant):
    ...
    def test_gptq_with_specified_matmul(self):

        algo_config = matmul_nbits_quantizer.GPTQWeightOnlyQuantConfig(calibration_data_reader=self.matmul_data_reader)
